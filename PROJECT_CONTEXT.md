...
12. Open **Usage restrictions** to configure either Restricted Mode or Brick Mode. Restricted Mode preserves the former Brick Mode behavior with an unrestricted-size allow-list. Brick Mode has a separate allow-list capped at eight user-chosen apps and locks that list while Brick Mode is active. Clock, Digital Detox, Digital Wellbeing, Google Play Store, Google Wallet, Lock My Phone, Messages, Niagara Launcher, Outlook, Phone, Settings, TimeHUD, and detected Home launchers are protected automatically when installed and do not count toward the cap. Either level can stay on until manually disabled, run for a one-minute-to-seven-day countdown, or be selected by recurring weekly schedules with local start times and durations up to 24 hours. Existing settings and legacy schedules migrate to Restricted Mode; overlapping sessions apply Brick Mode. Usage restrictions only cover visible unchosen launcher apps; they do not stop background services or system processes (`ui/brick/`, `blocking/BrickMode.kt`).
...
- Searchable Restricted Mode and Brick Mode allow-lists with an eight-app cap for Brick Mode, an off-by-default manual switch, persisted automatic-stop timer/live countdown, per-mode recurring weekly schedules, legacy-to-Restricted migration, and protected essential/Home/Settings apps.
...
| Navigation | One launcher activity with a state-based Material 3 navigation drawer. Goals is the default destination; App usage, Usage restrictions, App limits, and Permissions are sibling destinations. No Navigation Compose graph or route arguments are needed. |
| Dependency injection | None. Android services are acquired through `Context`; helper objects are Kotlin singletons. |
| Persistence | Android `SharedPreferences` for goals, service state, bubble position, Restricted/Brick Mode settings, app-limit rules, and daily focused-use totals; no Room, DataStore, files, or cache database. |
...
AccessibilityWindowInfo trees + usage-restriction/app-limit SharedPreferences
...
| `timehud_brick_mode` | `enabled`, `mode`, `allowed_packages`, `brick_allowed_packages`, `ends_at_epoch_ms`, `schedules` | `BrickModeSettings` | Off-by-default selected restriction level, separate Restricted/Brick allow-lists, timed deadline, and typed weekly schedules. The legacy `allowed_packages` key remains the Restricted Mode list, and legacy schedules without a mode decode as Restricted Mode. Protected essentials are derived rather than persisted. |
...
- Local Brick Mode with a searchable launchable-app allow-list, persisted optional deadline/live countdown, multiple enabled/disabled weekly schedules, protected reference apps/Home/Settings, and fail-open handling for background-only packages.
...
| `BrickModeTest.kt` / `BrickModeScreenTest.kt` | Brick Mode policy, timer, schedule, and search tests | Protects timed expiry, recurring/cross-midnight schedule windows, schedule codec/boundaries, duration/time validation, essential/chosen/background decisions, screenshot-name coverage, and name/package filtering | `BrickModeDecisionEngine`, `BrickModeTimer`, `BrickModeSchedulePolicy`, `BrickModeScheduleCodec`, `EssentialAppPolicy`, `filterBrickModeApps` |
