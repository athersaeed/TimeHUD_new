...
- Searchable Brick Mode allow-list with an off-by-default master switch, persisted automatic-stop timer, live countdown, and protected essential/Home/Settings apps.
...
- Local Brick Mode with a searchable launchable-app allow-list, persisted optional deadline/live countdown, protected reference apps/Home/Settings, and fail-open handling for background-only packages.
...
| `BrickModeTest.kt` / `BrickModeScreenTest.kt` | Brick Mode policy, timer, and search tests | Protects timed expiry, duration validation/formatting, essential/chosen/background decisions, screenshot-name coverage, and name/package filtering | `BrickModeDecisionEngine`, `BrickModeTimer`, `EssentialAppPolicy`, `filterBrickModeApps` |
