...
- The active window is created with `FLAG_NOT_FOCUSABLE`, so it does not own normal activity back handling (`OverlayService.kt:201-207`). Closing is through its button after a five-second delay.
...
| **Medium** | Full-screen interruption has weak escape behavior | It covers all apps and intercepts touches, while close is disabled for five seconds and normal Back is not owned by the overlay window. A UI regression can trap input until the service is stopped externally. |
