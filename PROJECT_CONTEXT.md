...
12. Open **Brick mode** to search installed launchable apps and choose the few that remain available. Clock, Digital Detox, Digital Wellbeing, Google Play Store, Google Wallet, Lock My Phone, Messages, Niagara Launcher, Outlook, Phone, Settings, TimeHUD, and detected Home launchers are protected automatically when installed. Brick Mode is off by default and can stay on until manually disabled, run for a one-minute-to-seven-day countdown, or use multiple recurring weekly schedules with selected weekdays, local start times, and durations up to 24 hours. Schedules reuse the same allow-list and may cross midnight. Brick Mode only covers visible unchosen launcher apps; it does not stop background services or system processes (`ui/brick/`, `blocking/BrickMode.kt`).
...
- Searchable Brick Mode allow-list with an off-by-default manual switch, persisted automatic-stop timer/live countdown, multiple recurring weekly schedules, and protected essential/Home/Settings apps.
